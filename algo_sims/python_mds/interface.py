...
def generate_similarities():
    ...
    for i in range(AVERAGING_FACTOR):
        noise = np.random.normal(loc=0, scale=1.5, size=(num_nodes, num_nodes)) #NxN
...
def gen_audio_data(n):
    global counter
    if n == 1:
        values = [33, 20, 28, 27, 29, 28, 29, 25, 30, 24, 37, 40, 20, 34, 88, 86, 78, 86, 80, 74, 80, 79, 71, 81, 88, 88, 89, 72, 72, 74]
        out = values[counter]
    if n == 2:
        values = [32, 27, 23, 33, 31, 13, 31, 40, 13, 34, 43, 9, 18, 23, 67, 78, 83, 60, 68, 79, 67, 71, 74, 70, 75, 79, 68, 65, 58, 66]
        out = values[counter]
        counter += 1
    return out
...
def single_mds_iter(X_true, similarities):
    ...
    for n in range(len(pos)):
        if n != 0: #skip anchor
            x = pos[n][0]
            y = pos[n][1]
            plt.text(x, y-0.75, 'Audio level (%): {}'.format(gen_audio_data(n)))

    bounds = 5 * METER_SCALING
